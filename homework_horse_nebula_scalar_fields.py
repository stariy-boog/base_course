...
def bell_function(x, y, intensity=1, dec_rate=[0.005, 0.005]):
    scalar_field = intensity*np.exp(-dec_rate[0] * x**2 - dec_rate[1] * y**2)
    return scalar_field

intensity_centrums_x = [160, 240]
intensity_centrums_y = [75, 100]
intensity_values = [100, 60]

def scalar_function(x, y, int_cen_x, int_cen_y, int_vel):
    scalar_field = 0
    for i in range(0, len(int_cen_x)):
        scalar_field += int_vel[i] * bell_function(x - int_cen_x[i], y - int_cen_y[i], 30, [100, 100])
    return scalar_field
